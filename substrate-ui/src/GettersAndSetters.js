...
export default function GettersAndSetters (props) {
  ...
  const staticEmail = <Static label='Email:' value={dbEmail}/>;
  ...
  function dynEmail (enable) {
    return (
        <Field
            label='Email Address:'
            value={inputEmail}
            criteria={emailValidity}
    ...
    return (
      <div className="card">
        ...
        <form>
          ...
          {dynamic ? dynEmail(enableField) : staticEmail}
        </form>
      ...
      if (result.isNone) {
        ...
        json.forEach(cert => {
          ...
          if (cert.version_number !== 0) {
            ...
            const EmailField = <Static label='Email:' value={cert.email}/>;
